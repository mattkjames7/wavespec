...
def _WFCosineBell(v,P=10.0):
	...
	out=np.array(v)
	i=np.array(range(0,t_len))
	out[i]=v[i]*(0.5+(0.5*np.cos(i*theta + np.pi)))
	
	i=np.array(range(l-1-t_len,l))
...
def _WFCosine(v,perc=10.0):
	
	l=np.size(v)
	out=np.array(v)	

	
	t_len=np.int32(perc*l/100.0)
	theta=(np.pi/np.float32(t_len))
...
def _WFGaussian(v,sigma=0.5):

	l=np.size(v)
	out=np.array(v)	
	i=np.array(range(0,l))
	out[i]=v[i]*(np.exp(-0.5*(((i-(l-1)/2.0)/(sigma*(l-1)/2.0))**2)))
